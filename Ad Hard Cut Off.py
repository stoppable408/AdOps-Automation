...
from v3modules import UtilUtils, CampaignUtils, AdUtils, MailUtils
...
for advertiser in advertiserSet:
    ...
    listValues = {"active":True, "advertiserId":currentAdvertiser}
    adList = AdUtils.listAd(Api, listValues)
    adList = [x for x in adList if ("11:59" in x["endTime"] or "3:59" in x["endTime"]) and "Brand-neutral" not in x['name'] and "TRACKING" not in x["name"] and "AD_SERVING_DEFAULT_AD" not in x["type"] and "AD_SERVING_TRACKING" not in x["type"]]
...
if len(finalAdList) > 0:
    ...
    MailUtils.send_email(['Hard Cut Off Report.xlsx'], "Ad Hard Cut Off Report", "Attached is a report containing all active ads that are still active that have their Hard-Cut off off", ["Lennon.Turner@carat.com","Holly.Champoux@carat.com"])
